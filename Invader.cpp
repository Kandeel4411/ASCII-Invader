...
{
	for (int i = 0; i < static_cast<int>(fireMonster.size()); i++)
	{
	...
	for (int i = 0; i < static_cast<int>(fireMonster.size()); i++)
	{
		for (int j = 0; j < static_cast<int>(firePlayer.size()); j++)
		{
			for (int k = 1; k >= 0; k--)
			{
...
{
	for (int i = 0; i < static_cast<int>(fireBoss.size()); i++)
	{
...
{
	for (int i = 0; i < static_cast<int>(firePlayer.size()); i++)
	{
		if (firePlayer[i].x <= 0)
		{
		...
		for (int j = 0; j < static_cast<int>(monsterShip.size()); j++)
		{
			for (int k = 2; k >= 0; k--)
			{
